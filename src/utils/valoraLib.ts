 ...
 * @param url
 */
export const parseDappkitResponse = (url: string) => {
...
export const removeQueryParams = (url: string, keys: any[]) => {
  const whereQuery = url.indexOf("?");
  if (whereQuery === -1) {
    return url;
  }
  const searchNonDeduped = url.slice(whereQuery + 1);
  const allSearch = searchNonDeduped.split("?");
  const newQs: { [p: string]: any; [p: number]: any } = allSearch.reduce(
    (acc, qs) => ({
      ...acc,
...
export const requestValoraAuth = async (): Promise<AccountAuthResponseSuccess> => {
  ...
  window.location.href = removeQueryParams(window.location.href, ["requestId", "type", "status", "address", "phoneNumber", "pepper"]);
 ...
 */
export const requestValoraTransaction = async (kit: ContractKit, txs: TxToSignParam[]): Promise<SignTxResponseSuccess> => {
...
const parseSearchParamsHashAware = (url: string): URLSearchParams => {
  ...
  const searchNonDeduped = url.slice(whereQuery + 1);
  const allSearch = searchNonDeduped.split("?");
  const newQs = allSearch.filter(identity).reduce((acc, qs) => ({ ...acc, ...Object.fromEntries(new URLSearchParams(qs).entries()) }), {});
